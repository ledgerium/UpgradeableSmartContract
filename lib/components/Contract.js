...
timeout() {
  return new Promise((resolve, reject) => {
    setTimeout(()=>{
      resolve('timeout')
    }, parseInt(process.env.TRANSACTION_TIMEOUT || 15000))
  })
}
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        .then(data => {
          ...
          logger.info(`Signing transaction with privateKey: 0x${process.env.PRIVATE_KEY}`)
          this.web3.eth.accounts.signTransaction(transactionParams, `0x${process.env.PRIVATE_KEY}`)
